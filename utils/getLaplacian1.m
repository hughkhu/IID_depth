function A=getLaplacian1(I,consts,epsilon,win_size)
   % consts=zeros(h,w);epsilon=0.1^5;
  ...
  neb_size=(win_size*2+1)^2;%9
  [h,w,c]=size(I);
  n=h; m=w;
  img_size=w*h;
  consts=imerode(consts,ones(win_size*2+1));%腐蚀
  
  indsM=reshape([1:img_size],h,w);

  tlen=sum(sum(1-consts(win_size+1:end-win_size,win_size+1:end-win_size)))*(neb_size^2);%前半段为const为0个数
  ...
  for j=1+win_size:w-win_size
    for i=win_size+1:h-win_size
      ...
      winI=winI-repmat(win_mu',neb_size,1);%去均值
